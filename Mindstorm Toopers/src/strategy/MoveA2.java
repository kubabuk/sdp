...
public class MoveA2 {
	...
	public static void makeCommands(World w, Goal goal, Queue aq) {
		...
		System.out.println("Command: " + goal.getMove());
		System.out.println("Point: " + goal.getGoal());
		...
		if (name.equals(CommandNames.CATCH)) {
			...
			if (!rightq && !hardboundarycheckleft(w,movetopoint.getDestination())) {
				System.out.println("Not in Left Quadrant");
				aq.add(new Command(CommandNames.DONOTHING, 0, 0));
				return;
			} else if (rightq && !hardboundarycheckright(w,movetopoint.getDestination())) {
				System.out.println(movetopoint.getDestination());
				System.out.println("Not in Right Quadrant");
				aq.add(new Command(CommandNames.DONOTHING, 0, 0));
				return;
			} else {
